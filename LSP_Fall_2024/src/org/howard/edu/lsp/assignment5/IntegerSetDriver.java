package org.howard.edu.lsp.assignment5;

import java.util.ArrayList;
import java.util.List;

public class IntegerSetDriver {
    public static void main(String[] args) {
        IntegerSet set1 = new IntegerSet();
        set1.add(1);
        set1.add(2);
        set1.add(3);
        System.out.println("Value of Set1 is: " + set1.toString());

        System.out.println("Smallest value in Set1 is: " + set1.smallest());
        System.out.println("Largest value in Set1 is: " + set1.largest());

        IntegerSet set2 = new IntegerSet();
        set2.add(3);
        set2.add(4);
        set2.add(5);
        System.out.println("Value of Set2 is: " + set2.toString());

        System.out.println("Union of Set1 and Set2");
        set1.union(set2);
        System.out.println("Result of union of Set1 and Set2: " + set1.toString());

        set1.clear();
        System.out.println("Set1 after clear: " + set1.toString());
        
        set1.add(1);
        set1.add(2);
        System.out.println("Set1 after adding 1 and 2: " + set1.toString());
        
        set1.remove(1);
        System.out.println("Set1 after removing 1: " + set1.toString());

        System.out.println("Set1 contains 2: " + set1.contains(2));
        System.out.println("Set1 contains 3: " + set1.contains(3));

        System.out.println("Testing intersection of Set1 and Set2");
        set1.intersect(set2);
        System.out.println("Result of intersection: " + set1.toString());

        set1.add(2);
        set1.add(3);
        System.out.println("Testing difference of Set1 and Set2");
        set1.diff(set2);
        System.out.println("Result of difference: " + set1.toString());

        set1.add(1);
        set1.add(2);
        System.out.println("Testing complement of Set1 with Set2");
        set1.complement(set2);
        System.out.println("Result of complement: " + set1.toString());

        System.out.println("Testing if Set1 is empty: " + set1.isEmpty());
        set1.clear();
        System.out.println("Set1 after clear, is empty: " + set1.isEmpty());

        System.out.println("Testing equality of two sets");
        set1.add(1);
        set2.add(1);
        System.out.println("Set1 equals Set2: " + set1.equals(set2));

        System.out.println("Final Set1: " + set1.toString());
    }
}
